...
const staticBackend = {
  ...
  async bust(payload) {
    ...
    if (!user) throw new Error('Not signed in');
    const now = new Date();
    const row = { user_id: user.id, timestamp: now.toISOString(), note: String(payload.note || '').slice(0, 240), temp_f: payload.temp_f, pressure: payload.pressure, lat: payload.lat, long: payload.long, city: payload.city, time_bucket: timeBucket(now) };
