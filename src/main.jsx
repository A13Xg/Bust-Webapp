...
import { Bell, ChevronUp, Gauge, LogOut, MapPin, Medal, Pencil, Repeat2, Sparkles, Thermometer, Trophy, Volume2, VolumeX, X } from 'lucide-react';
...
function progressGoal(item){ const text=`${item.desc||''} ${item.name||''}`; const n=text.match(/\b(\d+)\b/); if(item.id==='xp_tycoon') return 2000; if(/all five/i.test(text)) return 5; if(/all 7/i.test(text)) return 7; if(/everything else/i.test(text)) return achievements.length-1; return n?Number(n[1]):Number(item.goal)||1; }
