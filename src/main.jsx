...
function Overlay({title,onClose,children,showScrollTop=false}){ const ref=useRef(null); const isAnalytics=title==='ANALYTICS DRAWER'; return <motion.section ref={ref} className={`overlay${isAnalytics?' analytics-overlay':''}`} initial={{y:'100%'}} animate={{y:0}} exit={{y:'100%'}}><button className="close" onClick={onClose}><X/></button><div className="overlay-head"><h1>{title}</h1></div>{children}{showScrollTop&&<button type="button" className="scroll-top" aria-label="Back to top" title="Back to top" onClick={()=>ref.current?.scrollTo({top:0,behavior:'smooth'})}><ChevronUp/></button>}</motion.section> }
